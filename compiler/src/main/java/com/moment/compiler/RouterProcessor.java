package com.moment.compiler;

import com.google.auto.service.AutoService;
import com.moment.annotatiom.KtRouter;
import com.moment.annotatiom.Module;
import com.moment.annotatiom.Modules;
...
public class RouterProcessor extends AbstractProcessor {
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        ret.add(KtRouter.class.getCanonicalName());
        return ret;
    }
    ...
    private void generateDefaultRouterInit() {
        ...
        TypeSpec routerInit = TypeSpec.classBuilder("RouterInit")
                ...
                .build();
        try {
            JavaFile.builder("com.moment.ktrouter.router", routerInit)
                    .build()
                    .writeTo(filer);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private boolean handleRouter(String genClassName, RoundEnvironment roundEnv) {
        Set<? extends Element> elements = roundEnv.getElementsAnnotatedWith(KtRouter.class);

        MethodSpec.Builder mapMethod = MethodSpec.methodBuilder("map")
                .addModifiers(Modifier.PUBLIC, Modifier.FINAL, Modifier.STATIC)
                .addStatement("java.util.Map<String,String> transfer = null")
                .addStatement("com.moment.ktrouter.router.ExtraTypes extraTypes")
                .addCode("\n");

        for (Element element : elements) {
            KtRouter router = element.getAnnotation(KtRouter.class);
            ...
            mapMethod.addStatement("extraTypes = com.moment.ktrouter.router.ExtraTypes()");
            ...
            for (String format : router.value()) {
                ...
                if (element.getKind() == ElementKind.CLASS) {
                    mapMethod.addStatement("com.moment.ktrouter.router.Routers.map($S, $T.class, null, extraTypes)", format, className);
                } else {
                    mapMethod.addStatement("com.moment.ktrouter.router.Routers.map($S, null, " +
                            "new MethodInvoker() {\n" +
        ...
        TypeSpec routerMapping = TypeSpec.classBuilder(genClassName)
                ...
                .build();
        try {
            JavaFile.builder("com.moment.ktrouter.router", routerMapping)
                    .build()
                    .writeTo(filer);
        } catch (Throwable e) {
            e.printStackTrace();
